...
class InferenceService:
    def __init__(self, adapter_dir: Path):
        ...
        self._max_new_tokens = _settings.max_new_tokens
        self._num_beams      = _settings.num_beams
        base_model_id        = _settings.base_model
    ...
    def generate_from_prompt(
        ...
        """Tokenize a pre-built prompt and run inference."""
        ...
        with self._torch.no_grad():
            output = self.model.generate(
                **inputs,
                ...
                num_beams=num_beams or self._num_beams,
                early_stopping=True,
    ...
    def generate(
        ...
        """Build a prompt from parts then run generate_from_prompt."""
        if retrieved:
            ...
            prompt = (
                ...
                if rag
                else _format_prompt(question, document_text, visa_type, document_type, category, language)
            )
        else:
            prompt = _format_prompt(question, document_text, visa_type, document_type, category, language)

        return self.generate_from_prompt(prompt, max_new_tokens, num_beams)
